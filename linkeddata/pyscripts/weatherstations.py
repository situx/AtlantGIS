...
with open('../../raster/precipitation/atlantgis_weather_stations.csv', newline='') as csvfile:
    ...
    for row in wsreader:
        ...
        g.add((URIRef("http://atlantgis.squirrel.link/data/weatherstation/ws"+row["ws_id"]+"_2017_precipitation"),URIRef("http://www.w3.org/2000/01/rdf-schema#label"),Literal("Observation of precipitation value at weather station "+str(row["ws_id"])+" in the year 2017",lang="en")))
        g.add((URIRef("http://atlantgis.squirrel.link/data/weatherstation/ws"+row["ws_id"]+"_2017_precipitation"),URIRef("http://www.w3.org/2000/01/rdf-schema#label"),Literal("Beobachtung der Niederschlagsmenge an Wetterstation "+str(row["ws_id"])+" im Jahr 2017",lang="de")))
        ...
        g.add((URIRef("http://atlantgis.squirrel.link/data/weatherstation/ws"+row["ws_id"]+"_geom"),URIRef("http://www.opengis.net/ont/geosparql#asWKT"),Literal("POINT("+str(row["lon"]).replace(",",".")+" "+str(row["lat"]).replace(",",".")+")",datatype="http://www.opengis.net/ont/geosparql#wktLiteral")))
